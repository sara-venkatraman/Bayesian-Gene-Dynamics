...
geneSubset <- sample(geneNames, size=200)
...
Draw.R2.Scatterplot <- function(matrix1, matrix2, priorMatrix, geneSubset) {
  ...
  prior <- Vectorize.Labeled.Square.Matrix((priorMatrix[geneSubset, geneSubset] > 0) + 0)
  ...
  p <- ggplot(plotData, aes(x=x.axis, y=y.axis, color=prior, text=hoverText)) + 
    ...
    ggtitle("Empirical Bayes R^2 Values") + theme_light() + theme(legend.position="none") +
